...
function sm.regui.modconfig.cl_config_open()
    local gui = sm.regui.new("$CONTENT_DATA/Gui/Layouts/Options.relayout")
    local fullscreenGui = sm.regui.fullscreen.createFullscreenGuiFromInterface(gui, true, "Center")
    gui = fullscreenGui:getGui()
...
if sm.regui.modconfig.cl_config_hook_command then
    return
end

function sm.regui.modconfig.cl_config_hook_command(toolSelf, event)
    sm.regui.modconfig[event]()
end
...
sm.event.sendToWorld = function(world, callback, params)
    ...
    if params[1] == selectedCfgCmdName then
        if sm.isHost then
            sm.event.sendToTool(sm.regui.__getToolInstance().tool, "sv_config_hook_command", "cl_config_open")
        else
...
print("[ModConfig] Loaded ModConfig.lua")

print("Loaded ModConfig.lua")
